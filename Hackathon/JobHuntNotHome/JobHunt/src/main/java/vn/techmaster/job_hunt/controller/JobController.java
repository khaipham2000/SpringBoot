...
public class JobController {
  ...
  public String listAllJob(Model model) {
    model.addAttribute("searchRequest", new SearchRequest("",City.HaNoi));
    model.addAttribute("jobs", jobRepo.getAll());
    model.addAttribute("employer",empRepo.getAllEmployer());
    model.addAttribute("totalApplicantMap",applicantRepo.countApplicant());
    return "job_home";
  }
  ...
  public String searchKeyCity(@RequestBody @ModelAttribute("searchRequest") SearchRequest searchRequest, Model model){
    ...
    model.addAttribute("employer", empRepo.getMapEmp());
    model.addAttribute("totalApplicantMap", applicantRepo.countApplicant());
    return "job_home";
  }
  ...
  @GetMapping(value = "/{id}")
  public String showJobDetailByIDApplicant(Model model, @PathVariable String id) {
    Job job = jobRepo.findById(id);
    model.addAttribute("job", job);
    model.addAttribute("employer", empRepo.findById(job.getEmp_id()));
    return "job_detail";
